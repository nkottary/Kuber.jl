...
@doc raw"""Volume represents a named volume in a pod that may be accessed by any container in the pod.
    ...
    - csi::IoK8sApiCoreV1CSIVolumeSource : CSI (Container Storage Interface) represents ephemeral storage that is handled by certain external CSI drivers (Beta feature).
...
mutable struct IoK8sApiCoreV1Volume <: SwaggerModel
    ...
    ephemeral::Any # spec type: Union{ Nothing, IoK8sApiCoreV1EphemeralVolumeSource } # spec name: ephemeral
    ...
    function IoK8sApiCoreV1Volume(;awsElasticBlockStore=nothing, azureDisk=nothing, azureFile=nothing, cephfs=nothing, cinder=nothing, configMap=nothing, csi=nothing, downwardAPI=nothing, emptyDir=nothing, ephemeral=nothing, fc=nothing, flexVolume=nothing, flocker=nothing, gcePersistentDisk=nothing, gitRepo=nothing, glusterfs=nothing, hostPath=nothing, iscsi=nothing, name=nothing, nfs=nothing, persistentVolumeClaim=nothing, photonPersistentDisk=nothing, portworxVolume=nothing, projected=nothing, quobyte=nothing, rbd=nothing, scaleIO=nothing, secret=nothing, storageos=nothing, vsphereVolume=nothing)
        ...
        validate_property(IoK8sApiCoreV1Volume, Symbol("ephemeral"), ephemeral)
        setfield!(o, Symbol("ephemeral"), ephemeral)
